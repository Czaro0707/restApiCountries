...
import { toggleTheme } from "./ToggleTheme.mjs";
  ...
  .then((data) => {
    for (let i = 0; i < data.length; i++) {
      ...
      for (const i in cardData.currencies) {
        const currency =
          cardData.currencies[Object.keys(cardData.currencies)[indexCurrency]]
            .name;
        indexCurrency++;
      ...
      for (const i in cardData.languages) {
        const language =
          cardData.languages[Object.keys(cardData.languages)[indexLanguages]]
            .name;
        indexLanguages++;
