...
class Filter {
  ...
  changebyCountryName(e) {
    ...
    cards.forEach((card) => {
      if (
        (card.country.toUpperCase().includes(typed) &&
          card.region.toUpperCase() ===
            this.regionFilter.value.toUpperCase()) ||
...
export const filter = new Filter(
  document.querySelector(".filter__country__input"),
  document.querySelector(".filter__region__select")
);
